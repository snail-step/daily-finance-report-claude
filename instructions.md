...
## Step 1 — Prices
1. Fetch CURRENT price + 1-day % change for **every** ticker in the universe in **one parallel batch** via the FMP tools available in the current Codex session. Do not fetch one at a time. If FMP is unavailable, use live web price sources for the full universe and label every resulting price with `(web)` and its date. Do not stop an unattended scheduled run solely because FMP is unavailable.
2. Fallback only when a result is bad (do not web-search tickers FMP already returned cleanly):
   ...
   - Taiwan tickers (`.TW`): `Codex live web search("{TICKER} 股價 site:tw.stock.yahoo.com OR site:cnyes.com")`
   - Others: `Codex live web search("{TICKER} current price")`
...
## Git workflow — Codex desktop
- Work in a clean dedicated checkout. Fetch origin and fast-forward `main` before generating the report; stop on conflicts or unrelated local changes.
- Write reports/{YYYY-MM-DD}-brief.md using the Taiwan date. If today's report exists, report that fact and ask before replacing it.
- For an unattended scheduled run, if today's report already exists, leave it unchanged and finish successfully.
- Commit only the newly generated report and push `main` using ordinary Git. Do not force-push or merge unrelated work.
- Use the configured Git author identity. Do not add a Claude co-author trailer or invent an author identity.
- Verify the remote main commit after pushing; provide the report link. If push fails, retain the local report and explain the failure.

## Hard constraints (do NOT violate)
- Do NOT open a pull request or create a GitHub issue.
- Do NOT run gh or GitHub API commands. Ordinary git fetch/commit/push is permitted after the initial confirmation.
- Use the session's live web search tools for the existing searches; adapt tool syntax and batch size to the available interface.
- Preserve all tracked tickers, news windows, sentiment rules and English/Traditional Chinese report structure above.
- Cite news and fallback prices with normal Markdown source links and their dates. Do not invent missing data or treat failed searches as proof that no material news exists.
